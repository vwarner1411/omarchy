...
run_power_command() {
  ...
  : >"$call_log"
  PATH="$mock_bin:$ROOT/bin:$PATH" CALL_LOG="$call_log" "$ROOT/bin/omarchy-system-$action"
}
...
for action in reboot shutdown; do
  : >"$call_log"
  if PATH="$mock_bin:$ROOT/bin:$PATH" CALL_LOG="$call_log" FAIL_SYSTEMD_RUN=true "$ROOT/bin/omarchy-system-$action"; then
