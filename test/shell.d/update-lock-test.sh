...
OMARCHY_UPDATE_LOGGED=1 TEST_MARKER="$update_snapshot_marker" run_with_lock_env bash "$update_under_test" -y >"$test_tmp/update-first.out" 2>&1 &
update_pid=$!
...
set +e
OMARCHY_UPDATE_LOGGED=1 TEST_MARKER="$test_tmp/update-second-snapshot-started" run_with_lock_env bash "$update_under_test" -y >"$test_tmp/update-second.out" 2>&1
...
OMARCHY_UPDATE_LOGGED=1 TEST_MARKER="$keyring_marker" INHIBIT_PID_FILE="$inhibit_pid_file" \
  run_with_lock_env bash "$update_under_test" -y >"$test_tmp/update-inhibit.out" 2>&1 &
inhibit_update_pid=$!
...
OMARCHY_UPDATE_LOGGED=1 run_with_lock_env bash "$update_under_test" -y
...
OMARCHY_UPDATE_LOGGED=1 EXPECT_STAY_AWAKE=1 run_with_lock_env bash "$update_under_test" -y
